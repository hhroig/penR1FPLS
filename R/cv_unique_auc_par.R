#' Crossvalidation in parallel (same lambda for all components) using AUC.
#'
...
#'
#' @return A list of crossvalidates erros (mean AUCs) and penalties giving the minimum
#' meanAUCs per number of components.
#' @export
#'
...
cv_unique_auc_par <- function(X,
  ...
  for (ncomp_i in 1:ncomp) {
    ...
    AUC_lambda_fold <- foreach::foreach (i = 1:num_folds,
                                         .packages = c("pROC"),
                                         .combine = "cbind") %:%
    ...
    # Save the folds-averaged CV-AUC:
